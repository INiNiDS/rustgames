...
struct TransitionUniform { progress: f32, type_id: u32, dir_x: f32, dir_y: f32 }
...
impl TransitionRenderer {
    ...
    const fn bind_group_entries() -> [wgpu::BindGroupLayoutEntry; 5] {
        [
            Self::texture_entry(0), Self::sampler_entry(1),
            Self::texture_entry(2), Self::sampler_entry(3),
    ...
    fn create_pipeline(
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[bind_group_layout], immediate_size: 0,
        });
    ...
    fn build_render_pipeline(
        ...
        let vtx = wgpu::VertexState { module: shader, entry_point: Option::from("vs_main"),
            compilation_options: comp.clone(), buffers: &[] };
        let frag = wgpu::FragmentState { module: shader, entry_point: Option::from("fs_main"),
            compilation_options: comp, targets: &targets };
        device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Transition Pipeline"), layout: Some(layout),
            vertex: vtx, fragment: Some(frag), multiview_mask: None, cache: None,
    ...
    fn resolve_transition(transition: &Transition) -> (u32, f32, f32) {
        match transition {
            ...
            Transition::Wipe { direction, .. } => (4, direction.to_vector().x, direction.to_vector().y),
        }
    }
    fn create_render_bind_group<'a>(
        &'a self, device: &wgpu::Device,
        old_view: &'a wgpu::TextureView, new_view: &'a wgpu::TextureView,
    ) -> wgpu::BindGroup {
    ...
    pub fn render(
        ...
        old_view: &wgpu::TextureView, new_view: &wgpu::TextureView,
        queue: &wgpu::Queue, device: &wgpu::Device,
        transition: &Transition, progress: f32,
    ) {
        let (type_id, dir_x, dir_y) = Self::resolve_transition(transition);
        let uniform = TransitionUniform { progress, type_id, dir_x, dir_y };
    ...
    fn execute_pass(
        encoder: &mut wgpu::CommandEncoder, output_view: &wgpu::TextureView,
        pipeline: &wgpu::RenderPipeline, bind_group: &wgpu::BindGroup,
    ) {
