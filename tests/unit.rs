#[path = "unit/animation_tests.rs"]
mod animation_tests;
#[path = "unit/blizzard_soa_tests.rs"]
mod blizzard_soa_tests;
#[path = "unit/camera_advanced_tests.rs"]
mod camera_advanced_tests;
#[path = "unit/camera_tests.rs"]
mod camera_tests;
#[path = "unit/color_parsing_tests.rs"]
mod color_parsing_tests;
#[path = "unit/color_tests.rs"]
mod color_tests;
#[path = "unit/easing_tests.rs"]
mod easing_tests;
#[path = "unit/easing_transition_tests.rs"]
mod easing_transition_tests;
#[path = "unit/effects_tests.rs"]
mod effects_tests;
#[path = "unit/event_tests.rs"]
mod event_tests;
#[path = "unit/extended_tests.rs"]
mod extended_tests;
#[path = "unit/extra_stress_test.rs"]
pub mod extra_stress_test;
#[path = "unit/sprite_animation_tests.rs"]
mod sprite_animation_tests;
#[path = "unit/sprite_tests.rs"]
mod sprite_tests;
#[path = "unit/stress_tests.rs"]
mod stress_tests;
#[path = "unit/text_tests.rs"]
mod text_tests;
#[path = "unit/translation_tests.rs"]
mod translation_tests;
#[path = "unit/typewriter_tests.rs"]
mod typewriter_tests;
